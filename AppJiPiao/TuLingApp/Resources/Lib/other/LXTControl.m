...
+ (UIButton *) createBtnWithFrame:(CGRect)frame titleName:(NSString *)title imgName:(NSString *)img selImgName:(NSString *)selImg target:(id)target action:(SEL)action
{
    UIButton*button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = frame;
    [button setTitle:title forState:UIControlStateNormal];
...
+ (NSString *)returnDateHHAndDDStr:(NSString *)dateStr
{
    
    NSDateFormatter *judging_info_df = [[NSDateFormatter alloc] init];
    
    [judging_info_df setDateFormat:@"yyyy-MM-dd"];
    NSDate *date = [judging_info_df dateFromString:dateStr];
    
    [judging_info_df setDateFormat:@"yy-MM-dd"];
