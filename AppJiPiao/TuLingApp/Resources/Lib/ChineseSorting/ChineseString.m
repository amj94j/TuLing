    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        if ([predicate evaluateWithObject:initialStr])
        {
            ...
            if(![chineseString.string isEqualToString:@""]){
                ...
                if ([chineseString.string isEqualToString:@"重庆"]) {
                    pinYinResult=@"CQ";
                }
                if ([chineseString.string isEqualToString:@"长沙"]) {
                    pinYinResult=@"CS";
                }
                if ([chineseString.string isEqualToString:@"厦门"]) {
                    pinYinResult=@"XM";
                }
                if ([chineseString.string isEqualToString:@"沈阳"]) {
                    pinYinResult=@"SY";
                }
                if (pinYinResult.length==0) {
                for(int j=0;j<chineseString.string.length;j++){
                 NSString *a= [NSString stringWithFormat:@"%c",
                     pinyinFirstLetter([chineseString.string characterAtIndex:j])];
