...
-(void)requestLogin
{
    ...
    [NetAccess getJSONDataWithUrl:kuserLogin parameters:params WithLoadingView:YES andLoadingViewStr:@"正在登录" success:^(id responseObject) {
        ...
        if ([code isEqualToString:@"0"]) {
            ...
            [MobClick event:@"102"];
            //环信
            EMOptions *options = [EMOptions optionsWithAppkey:@"1151170228115558#kefuchannelapp24950"];
            //开发环境下推送证书名称
            //options.apnsCertName = @"aps_development";
            options.apnsCertName = apnsCert;
            //生产环境下推送证书名称
            //options.apnsCertName = @"aps";
    ...
    [NetAccess getJSONDataWithUrl:kotherLogin parameters:params WithLoadingView:YES andLoadingViewStr:@"正在登录" success:^(id responseObject) {
        ...
        if ([code isEqualToString:@"0"]) {
            ...
            }else
            {
                ...
                vc.dataDict = [responseObject objectForKey:@"date"];
