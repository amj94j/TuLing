...
//pageControl边距
static CGFloat pageControlMagin = 10.0;
...
- (void)initViewWithNumberOfPages:(NSInteger)numberOfPages configuration:(JYConfiguration *)config addInView:(UIView *)superView{
    ...
    if (self.config.pageContollType == LabelPageControl){
        if (!self.pageLabel && (self.numberOfPages > 1)) {
            self.pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.superViewFrame) - 20 - 34, CGRectGetHeight(self.superViewFrame) - 13 -44, 34, 34)];
        ...
        self.pageControl.frame = CGRectMake(0, CGRectGetHeight(self.superViewFrame) - PXChange(80), CGRectGetWidth(self.superViewFrame), 25);
