...
#define windowContentWidth  ([[UIScreen mainScreen] bounds].size.width-30)
...
-(instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titleArray clickBlick:(btnClickBlock)block{
    ...
    if (self) {
        ...
        _bgScrollView=[[UIScrollView alloc] initWithFrame:CGRectMake(15, 0, windowContentWidth, self.frame.size.height)];
        _bgScrollView.backgroundColor=[UIColor whiteColor];
        _bgScrollView.showsHorizontalScrollIndicator=NO;
        _bgScrollView.contentSize=CGSizeMake(_btn_w*titleArray.count, self.frame.size.height);
        if (_bgScrollView.contentSize.width > WIDTH){
            _isGreater = YES;
        }else{
            _isGreater = NO;
        }
...
//        [_bgScrollView addSubview:line];
        ...
        _selectLine=[[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-1.5, _btn_w, 1.5)];
        ...
        UIImageView * hintView = [[UIImageView alloc] initWithFrame:CGRectMake(self.frame.size.width - 15, 0, 15, self.frame.size.height)];
        hintView.image = [UIImage imageNamed:@"zhe.png"];
...
-(void)btnClick:(UIButton *)btn{
    ...
    [UIView animateWithDuration:.2 animations:^{
        ...
        _selectLine.frame=CGRectMake(btn.frame.origin.x, self.frame.size.height-1.5, btn.frame.size.width, 1.5);
...
-(void)updateView{
    for (UIButton *btn in _btns) {
        ...
        if (btn.tag-1==_defaultIndex-1) {
            ...
            btn.selected=YES;
            //计算偏移量
            CGFloat offsetX=_btn_w*(_defaultIndex-1) - 2*_btn_w;
            if (offsetX<0) {
                offsetX=0;
            }
            CGFloat maxOffsetX= _bgScrollView.contentSize.width-windowContentWidth;
            ...
            [_bgScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
            _selectLine.frame = CGRectMake(_btn_w*(_defaultIndex-1), self.frame.size.height-1.5, _btn_w, 1.5);
